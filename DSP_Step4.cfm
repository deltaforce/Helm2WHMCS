        ...
        <cfhttp url="#application.whmcs.api.url#" method="post" charset="utf-8">
            ...
            <cfif isJSON(cfhttp.filecontent)>
				...
				<cfif response.result is "error"><!--- there was an error from whmcs --->
                    <cfset ArrayAppend(result,Struct('error':'WHMCS API error','message':response.message,'Client': Struct('firstname':api.request.firstname,'lastname':api.request.lastname,'Company Name': api.request.companyname,'Email':api.request.email)))>
                <cfelse><!--- whmcs success --->
                    <cfset ArrayAppend(result,Struct('success':'','Client': Struct('firstname':api.request.firstname,'lastname':api.request.lastname,'Company Name': api.request.companyname,'Email':api.request.email)))>
            ...
            <cfelse><!--- response from whmcs was not JSON, so something is worng --->
            	<cfset ArrayAppend(result,Struct('error':'Could not deserialize response from WHMCS, therefore is was a valid JSON response','message':cfhttp.filecontent,'Client': Struct('firstname':api.request.firstname,'lastname':api.request.lastname,'Company Name': api.request.companyname,'Email':api.request.email)))>
            </cfif>            
...
<form method="post" action="index.cfm?step=4"  class="form-horizontal">
...
<div class="well">
    ...
    <cfoutput><p>Users to migrate : #ListLen(session.selectedUsers)#</p></cfoutput>
        ...
        <div  id="customEmailDiv" style="display:none">	
            ...
            <div class="controls">
              ...
              <textarea  name="emailMessage" id="emailMessage" style="width:450px;height:200px"><cfinclude template="customEmailTemplate.cfm"> </textarea>
                	...
                	<label class="checkbox" for="saveEmail">Save for future use
